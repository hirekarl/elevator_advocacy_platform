...
class SODAService:
    ...
    def get_elevator_complaints(
        ...
        """
        Fetches complaints for a specific BIN, filtered by elevator categories.
        """
        self.limiter.wait()
        # Active category codes: '6S' (elevator) and '6M' (elevator/escalator).
    ...
    def get_recent_outages(
        ...
        # SODA date_entered is MM/DD/YYYY text. We fetch recent and filter in Python.
        if hours == 0:
            ...
            try:
                ...
                if threshold:
                    ...
                    for row in data:
                        ...
                        if raw_date:
                            try:
                                dt = datetime.strptime(raw_date, "%m/%d/%Y").replace(
                                    tzinfo=timezone.get_current_timezone()
                                )
                                if dt >= threshold:
                                    chunk_processed.append(row)
                                else:
                                    # Generous sequential assumption check
    ...
    def get_district_bins(self, district_id: str) -> List[str]:
        ...
        Uses MapPLUTO (64uk-42ks) and Building Footprints (5zhs-2jue).
        """
        ...
        while True:
            ...
            if not data:
                break
            all_bbls.extend(
                [str(row["bbl"]).split(".")[0] for row in data if "bbl" in row]
            )
            if len(data) < limit:
                break
            offset += limit
    ...
    def get_management_data(self, bin: str) -> Dict[str, Optional[str]]:
        ...
        try:
            self.limiter.wait()
            f_params: Dict[str, Any] = {"bin": bin, "$limit": 1}
            f_resp = requests.get(FOOTPRINTS_URL, params=f_params, timeout=10)
            ...
            self.limiter.wait()
            p_params: Dict[str, Any] = {"bbl": bbl, "$limit": 1}
            p_resp = requests.get(PLUTO_URL, params=p_params, timeout=10)
            p_resp.raise_for_status()
            p_data = p_resp.json()
            if p_data:
                owner = p_data[0].get("ownername")
                return {
                    "management_company": owner,  # Owner as proxy for management
                    "owner_name": owner,
                }
    ...
    def get_city_stats(self) -> Dict[str, Any]:
        ...
        def _query_top_buildings() -> List[Dict[str, Any]]:
            fields = "house_number, house_street, community_board, bin"
            params: Dict[str, Any] = {
                "$select": f"{fields}, count(*) AS complaint_count",
                "$where": f"{category_filter} AND {two_year_filter}",
                "$group": fields,
        ...
        if not use_mock:
                ...
                try:
                    ...
                    if db_building.latitude and db_building.longitude:
                        district = district_from_coordinates(
                            float(db_building.latitude),
                            float(db_building.longitude),
                        )
            ...
            # Resolve rep names (one query per building, ≤15)
